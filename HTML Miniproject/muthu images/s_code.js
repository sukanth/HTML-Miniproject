...
/* if no friendly pagename is set on the page, set pageName based on URL folder structure*/
	...
	if (s.pageName.lastIndexOf('.html')>-1) s.pageName = s.pageName.substr(0, s.pageName.lastIndexOf('.html')); 
	...
	/* extrenal campaign */
	if(!s.campaign){ 
	s.campaign=s.getQueryParam('cmpid,semcmpid');
	}
	/* internal campaign */
	if(!s.eVar11){
	s.eVar11=s.getQueryParam('icmpid');
	s.prop11=s.eVar11;
	}
    /* affiliate id */
    if(!s.eVar27){
    s.eVar27=s.getQueryParam('affid');
    s.prop27=s.eVar27;
    }
	/* Internal Search Analysis */
	s.previousPage=s.getPreviousValue(s.pageName,'gpv_pn','');
  	if (s.eVar6){
		s.eVar6 =s.prop6=s.eVar6.toLowerCase()
		s.prop7=s.eVar7=s.previousPage; //search term location
		s.events=s.apl(s.events,"event6",",",0);
	}
	/* Set Page View Event */
	s.events=s.apl(s.events,'event1',',',2);
	
	s.prop1=s.eVar1  //site section
	s.prop2=s.eVar2 //sub site section
	s.prop3=s.eVar3 //page type
	if(s.eVar4){s.prop4=s.eVar4 + ':' + s.pageName;} //login status
	s.prop35=document.URL;

	if(s.campaign){
		s.prop28=s.campaign+':'+s.pageName;
		s.eVar28=s.prop28;
		s.prop29=s.campaign+':'+s.prop3;
		s.eVar29=s.prop29;
		s.prop30=s.campaign;
	}else{
		s.prop28=s.pageName;
	...
	if(s.events.indexOf('prodView')>-1){
		if(!s.products || s.products=="" || s.products==";"){
			s.prop34='prodView->empty prd value:' + document.URL;
		}
	}
	if(s.events.indexOf('purchase')>-1){
		if(!s.products || s.products=="" || s.products==";"){
			s.prop34='purchase->empty prd value:' + document.URL;
		}
	}
	if(s.events.indexOf('purchase')>-1){
		if(!s.purchaseID || s.purchaseID==""){
			s.prop34='purchase->empty purchaseID:' + document.URL;
		}
	}
	
	try{
		FKART.omnitureSetEventPathing(s);
	}catch(e){}
	
	/* get percentage pages viewed */
	s.prop26=s.previousPage
	if (s.prop26){s.prop27=s.getPercentPageViewed();}
...
function omnAddCompareProduct(anl_products) {
	var s=s_gi(s_account);
    s.linkTrackVars='products,events';
	s.linkTrackEvents='event25';
    s.events='event25';
    s.products = anl_products;
	s.tl(s,'o','Compare Product');
}

/* * Plugin: getPercentPageViewed v1.2 */s.getPercentPageViewed=new Function("",""+"var s=this;if(typeof(s.linkType)=='undefined'||s.linkType=='e'){var"+" v=s.c_r('s_ppv');s.c_w('s_ppv',0);return v;}");s.getPPVCalc=new Function("",""+"var s=s_c_il["+s._in+"],dh=Math.max(Math.max(s.d.body.scrollHeight,"+"s.d.documentElement.scrollHeight),Math.max(s.d.body.offsetHeight,s."+"d.documentElement.offsetHeight),Math.max(s.d.body.clientHeight,s.d."+"documentElement.clientHeight)),vph=s.wd.innerHeight||(s.d.documentE"+"lement.clientHeight||s.d.body.clientHeight),st=s.wd.pageYOffset||(s"+".wd.document.documentElement.scrollTop||s.wd.document.body.scrollTo"+"p),vh=st+vph,pv=Math.round(vh/dh*100),cp=s.c_r('s_ppv');if(pv>100){"+"s.c_w('s_ppv','');}else if(pv>cp){s.c_w('s_ppv',pv);}");s.getPPVSetup=new Function("",""+"var s=this;if(s.wd.addEventListener){s.wd.addEventListener('load',s"+".getPPVCalc,false);s.wd.addEventListener('scroll',s.getPPVCalc,fals"+"e);s.wd.addEventListener('resize',s.getPPVCalc,false);}else if(s.wd"+".attachEvent){s.wd.attachEvent('onload',s.getPPVCalc);s.wd.attachEv"+"ent('onscroll',s.getPPVCalc);s.wd.attachEvent('onresize',s.getPPVCa"+"lc);}");s.getPPVSetup();
